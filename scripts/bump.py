...
FILES: dict[str, list[Path]] = {
    "rust": [
        ROOT / "crates/docoxide/Cargo.toml",
    ],
    "python": [
...
def resolve(args: argparse.Namespace) -> tuple[dict[str, str], str]:
    ...
    if args.target == "all" and not can_bulk_bump(args.bump, currents):
        raise RuntimeError(
            f"cannot bulk bump '{args.bump}' across diverged versions {currents}; "
            "bump each target individually or pass an explicit version",
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bump the version of one or all docoxide bindings.")
